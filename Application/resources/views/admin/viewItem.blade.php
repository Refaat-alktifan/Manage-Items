...
@endif
 ...
 <hr>
  ...
  {{ $reply->render() }}
   ...
   <form class="ui reply form" method="POST" action="{{ url('admin/item/'.$item->tid) }}">
   {{ csrf_field() }}
  ...
  <div class="col-md-4"><a href="#message" class="btn  btn-block btn-primary">    <i class="fa fa-comment"></i>   Reply </a></div>
      @if ($item->status == 1)
      <div class="col-md-4"><a href="{{ url('item/open/'.$item->tid) }}" class="btn btn-block  btn-primary">    <i class="fa fa-retweet"></i>   Open </a>
      @else
      <div class="col-md-4"><a href="{{ url('item/close/'.$item->tid) }}" class="btn btn-block  btn-primary">    <i class="fa fa-close"></i>   Close </a>
      @endif

  </div>
<div class="col-md-4"><a href="{{ url('manage/item/delete/'.$item->tid) }}" class="btn btn-block  btn-danger">    <i class="fa fa-trash"></i>   Delete </a>
</div>
        </div>
